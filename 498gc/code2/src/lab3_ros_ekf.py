...
class listenerNode():
    ...
    def EKF(self):
        ...
        vdotx = np.dot(f, self.L_lb[:,0])
        vdoty = np.dot(f, self.L_lb[:,1])
        vdotz = np.dot(f, self.L_lb[:,2])
        vdot = np.array([vdotx, vdoty, vdotz])

        qdot = -0.5*(self.Big_Omega(self.p, self.q, self.r) @ self.qv)
        ...
        # Euler works fine
        self.xhat += xhatdot.reshape((16,1))*self.dt #TODO: Check this one
        ...
        #delV/delQ
        # F_vq = 2*np.array([
        #     [(self.q1*self.fx-self.q4*self.fy+self.q3*self.fz), (self.q2*self.fx+self.q3*self.fy+self.q4*self.fz), (-self.q3*self.fx+self.q2*self.fy+self.q1*self.fz), (-self.q4*self.fx-self.q1*self.fy+self.q2*self.fz)],
        #     [self.q4*self.fx_self.q1*self.fy-self.q2*self.fz, self.q3*self.ax]
        # ])
        F_vq = np.apply_along_axis(np.dot, 2, self.delv_delq(self.q1, self.q2, self.q3, self.q4), f) # 2, 3, shape=(3,4)
        
        #delV/del_abias
        F_vba = -self.L_bl # 2, 5, shape=(3,3)

        #delQ/delQ
        F_qq = -0.5*self.Big_Omega(self.p, self.q, self.r) # 3, 3, shape=(4,4)
        ...
        Phi = np.zeros((16, 16))
        Phi[0:3,3:6] = self.I # delp/delv = F_pv = I_{3x3}
        Phi[6:9, 6:10] = F_vq
        Phi[6:9, 13:16] = F_vba
        Phi[9:13, 9:13] = F_qq
        ...
        # P1 = Pdot*dt
        # P = P +Pdot*dt
        #TODO: Check this (slide 66/107)
        self.Pdot = Phi @ self.P + self.P @ Phi.T + self.Q
        # P1 = self.Pdot*self.dt
        # P2 = (self.Pdot+P1/2)*self.dt
        # P3 = (self.Pdot+P2/2)*self.dt
        # P4 = (self.Pdot+P3/2)*self.dt
        self.P += self.Pdot*self.dt # (P1+2*P2+2*P3+P4)/6 ??
        ...
        # del v/del q
        # char_matrix_Hxq = np.array([       # eq. (124), slide 95/107
        #     [-2*self.q1, -2*self.q2, 2*self.q3, 2*self.q4],
        #     [-2*self.q4, -2*self.q3, -2*self.q2, -2*self.q1],
        #     [2*self.q3, -2*self.q4, 2*self.q1, -2*self.q2]
        # ])
        # H_xq = char_matrix_Hxq * self.z[0]
        H_xq = np.array([
            [-2*self.q1*self.z[0], -2*self.q2*self.z[0],  2*self.q3*self.z[0],  2*self.q4*self.z[0]],
            [-2*self.q4*self.z[0], -2*self.q3*self.z[0], -2*self.q2*self.z[0], -2*self.q1*self.z[0]],
            [ 2*self.q3*self.z[0], -2*self.q4*self.z[0],  2*self.q1*self.z[0], -2*self.q2*self.z[0]]
        ]).reshape(3, -1)
        
        #del P/del q
        # char_matrix_Hvq = np.array([        # (125)
        #     [ 2*self.q3*self.q           + 2*self.q4*self.r,            2*self.q4*self.q           - 2*self.q3*self.r,           2*self.q1*self.q           - 2*self.q2*self.r, 2*self.q2*self.q + 2*self.q1*self.r],
        #     [-2*self.q2*self.q           - 2*self.q1*self.r,            2*self.q2*self.r           - 2*self.q1*self.q,           2*self.q4*self.q           - 2*self.q3*self.r, 2*self.q3*self.q + 2*self.q4*self.r],
        #     [ 2*self.q1*self.q           - 2*self.q2*self.r,           -2*self.q2*self.q           - 2*self.q1*self.r,          -2*self.q3*self.q           - 2*self.q4*self.r, 2*self.q4*self.q - 2*self.q3*self.r]
        # ])
        # H_xvq = char_matrix_Hvq * self.z[0]
        # H_xvq = np.array([
        #     [ 2*self.q3*self.q*self.z[0] + 3*self.q4*self.r          ,  2*self.q4*self.q*self.z[0] - 2*self.q3*self.r*self.z[0],  2*self.q1*self.q*self.z[0] - ...]
        #     [-2*self.q2*self.q*self.z[0] - 2*self.q1*self.r*self.z[0],  2*self.q2*self.r*self.z[0] - 2*self.q1*self.q*self.z[0], ...],
        #     [ 2*self.q1*self.q*self.z[0] - 2*self.q2*self.r*self.z[0], -2*self.q2*self.q*self.z[0] - 2*self.q1*self.r*self.z[0], -2*...]
        # ])
        H_xvq = np.array([
            [ 2*self.q3*self.q*self.z[0] + 2*self.q4*self.r*1.5      ,  2*self.q4*self.q*self.z[0] - 2*self.q3*self.r*self.z[0],  2*self.q1*self.q*self.z[0] - 2*self.q2*self.r*self.z[0], 2*self.q2*self.q*self.z[0] + 2*self.q1*self.r*self.z[0]],
            [-2*self.q2*self.q*self.z[0] - 2*self.q1*self.r*self.z[0],  2*self.q2*self.r*self.z[0] - 2*self.q1*self.q*self.z[0],  2*self.q4*self.q*self.z[0] - 2*self.q3*self.r*self.z[0], 2*self.q3*self.q*self.z[0] + 2*self.q4*self.r*self.z[0]],
            [ 2*self.q1*self.q*self.z[0] - 2*self.q2*self.r*self.z[0], -2*self.q2*self.q*self.z[0] - 2*self.q1*self.r*self.z[0], -2*self.q3*self.q*self.z[0] - 2*self.q4*self.r*self.z[0], 2*self.q4*self.q*self.z[0] - 2*self.q3*self.r*self.z[0]]
        ]).reshape(3, -1)
        ...
        # NOTE: H matrix is the same as C in the equations on slide 66

        #Compute Kalman gain
        L_k = self.P @ H.T @ LA.inv(H @ self.P @ H.T + self.R)
        
        #Perform xhat correction    xhat = xhat + K@(z-H@xhat)
        self.xhat = self.xhat + L_k @ (self.z - H @ self.xhat)
        
        #propagate error covariance approximation P = (np.eye(16,16)-K@H)@P
        self.P = (np.eye(16, 16) - L_k @ H) @ self.P
    ...
    def delv_delq(self, q1, q2, q3, q4):
        return 2*np.array([
